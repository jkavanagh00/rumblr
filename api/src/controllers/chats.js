...
async function addChat_controller(req, res, next) {
  try {
...
async function addMessage_controller(req, res, next) {
  try {
...
async function getChats_controller(req, res, next) {
  try {
    const chats = await getActiveChatsByUserId_model(req.user.id);
...
// Get message by Id function will be added here

async function getMessages_controller(req, res, next) {
  try {
    const rumbleId = req.params.id;
